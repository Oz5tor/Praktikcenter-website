...
if (isset($_POST['Submit']))
{
		...
		if ((isset($username)) or (isset($pass)))
	{
		...
		if (mysqli_num_rows($result)==1)
		{
            ...
            $roles_result=mysqli_query ($db_conn,$roles_sql) or die (mysqli_error());
            while ($roles_row = mysqli_fetch_assoc($roles_result))
			{
                ...
                $perm_id_result = mysqli_query ($db_conn,$perm_id_sql) or die (mysqli_error());
                while($perm_id_row = mysqli_fetch_assoc($perm_id_result))
                {
                    ...
                    $perm_result = mysqli_query ($db_conn,$perm_sql) or die (mysqli_error());
